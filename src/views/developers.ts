...
export function developersView({ user, clients = [] }: DeveloperProps): string {
  const content = `
    <div class="w-full max-w-4xl mx-auto space-y-6">
        ...
        <div>
          <div class="flex items-center gap-2">
            <h1 class="text-xl font-semibold text-zinc-900 dark:text-zinc-100 tracking-tight">
              Portal Pengembang (OAuth 2.1 & SSO)
            </h1>
            <span class="text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 px-2 py-0.5 rounded font-mono font-medium">RFC 7591 / OIDC</span>
          </div>
          <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
            Daftarkan dan kelola aplikasi satelit (misalnya *.ten.my.id) untuk mengaktifkan Single Sign-On.
          </p>
        </div>

        <button 
          id="open-modal-btn"
          class="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-zinc-50 dark:text-zinc-900 font-medium text-xs hover:bg-zinc-800 dark:hover:bg-zinc-200 transition shadow-sm"
        >
      ...
      <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm">
        <h2 class="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-4">
          Aplikasi Satelit Terdaftar (${clients.length})
        </h2>
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div class="text-xs text-zinc-500">
                          ID Database: <code class="font-mono text-[11px]">${escapeHtml(client.id)}</code>
                        </div>
                      </div>
                    </div>
        ...
        ` : `
          <div class="text-center py-10 text-xs text-zinc-400 dark:text-zinc-500 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-lg">
            Belum ada aplikasi satelit yang terdaftar. Klik "Daftarkan Aplikasi Baru" untuk memulai.
          </div>
        `}
      </div>

      <!-- Quick Integration Snippet -->
      <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 shadow-sm space-y-3">
        <h2 class="text-xs font-semibold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">
          Informasi Endpoint OIDC Standar
        </h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <div>
            <span class="text-zinc-500">Issuer / Discovery:</span>
            <div class="font-mono text-[11px] mt-0.5 text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-800 p-1.5 rounded border border-zinc-200 dark:border-zinc-700">
              https://accounts.ten.my.id/.well-known/openid-configuration
            </div>
          </div>
          <div>
            <span class="text-zinc-500">Authorization Endpoint:</span>
            <div class="font-mono text-[11px] mt-0.5 text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-800 p-1.5 rounded border border-zinc-200 dark:border-zinc-700">
              https://accounts.ten.my.id/api/auth/oauth2/authorize
            </div>
          </div>
          <div>
            <span class="text-zinc-500">Token Endpoint:</span>
            <div class="font-mono text-[11px] mt-0.5 text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-800 p-1.5 rounded border border-zinc-200 dark:border-zinc-700">
              https://accounts.ten.my.id/api/auth/oauth2/token
            </div>
          </div>
          <div>
            <span class="text-zinc-500">Userinfo Endpoint:</span>
            <div class="font-mono text-[11px] mt-0.5 text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-800 p-1.5 rounded border border-zinc-200 dark:border-zinc-700">
              https://accounts.ten.my.id/api/auth/oauth2/userinfo
            </div>
          </div>
        </div>
      </div>
    ...
    <div id="create-modal" class="hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 sm:p-8 max-w-lg w-full shadow-lg relative">
        ...
        <p class="text-xs text-zinc-500 dark:text-zinc-400 mb-5">
          Sistem akan membuatkan Client ID dan Client Secret secara otomatis.
        </p>
        ...
        <form id="create-client-form" class="space-y-4">
          ...
          <div class="flex items-center justify-end gap-3 pt-3">
            ...
            <button 
              ...
              class="px-4 py-2 rounded-lg bg-zinc-900 dark:bg-zinc-100 text-zinc-50 dark:text-zinc-900 font-medium text-xs hover:bg-zinc-800 dark:hover:bg-zinc-200 transition"
            >
              Daftarkan Aplikasi
            </button>
          </div>
  ...
  const scripts = `
    <script>
      ...
      form?.addEventListener('submit', async (e) => {
        ...
        try {
          const res = await fetch('/api/developers/clients', {
            method: 'POST',
          ...
          if (!res.ok) {
            ...
            submitBtn.textContent = 'Daftarkan Aplikasi';
            return;
          }
          ...
          alertBox.textContent = 'Aplikasi berhasil didaftarkan! Memuat ulang...';
          setTimeout(() => {
      ...
      document.querySelectorAll('.rotate-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          try {
            const res = await fetch('/api/developers/clients/' + id + '/rotate', { method: 'POST' });
            if (res.ok) {
              window.location.reload();
            } else {
      ...
      document.querySelectorAll('.delete-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
          ...
          try {
            const res = await fetch('/api/developers/clients/' + id, { method: 'DELETE' });
            if (res.ok) {
              window.location.reload();
            } else {
  ...
  return layout({ title: 'Portal Pengembang', user, content, scripts });
}
